...
#  analyze and genmodel_old immune receptors generation, selection, mutation and all
#  other processes.
...
class GenModel(object):
    """Generative genmodel_old class containing genmodel_old topology and realization
    ...
    def read_model_parms(self, filename):
        """Reads a genmodel_old graph structure from a genmodel_old params file.
    ...
    for event in model.events:
        if event.name == event_name:
            ...
            if event.event_type != "DinucMarkov":
                ...
                # Takes care of genmodel_old 1(non log part)
                for ev in event_cluster:
...
def read_marginals_txt(filename, dim_names=False):
    """Reads a genmodel_old marginals file. Returns a tuple containing a dict
